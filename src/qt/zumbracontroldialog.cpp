...
#include "ui_zumbracontroldialog.h"
...
std::list<std::string> ZUmbraControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZUmbraControlDialog::listMints;

ZUmbraControlDialog::ZUmbraControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZUmbraControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZUmbraControlDialog::~ZUmbraControlDialog()
{
    delete ui;
}

void ZUmbraControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZUmbraControlDialog::updateList()
{
...
void ZUmbraControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZUmbraControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZUmbra_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZUmbraControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZUmbraControlDialog::GetSelectedMints()
{
...
void ZUmbraControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
